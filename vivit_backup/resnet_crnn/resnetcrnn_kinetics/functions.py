...
# Add this new class to your functions.py file
class CustomDataset_CRNN(data.Dataset):
    """Dataset class that handles custom frame naming formats"""
    def __init__(self, data_path, frames, transform=None):
        """
        Initialization
        data_path: path to the directory containing frames
        ...
        self.all_files = sorted(os.listdir(data_path))

        # Group files by video (assuming they're named like frame_XXX, frame1_XXX, etc.)
        self.video_groups = {}
        for file in self.all_files:
            # Extract video ID (e.g., "", "1", "2", etc.)
            if file.startswith("frame_"):
                video_id = ""
            elif file.startswith("frame"):
                # Extract the number after "frame" and before "_"
                parts = file.split("_")
                if len(parts) > 1:
                    video_id = parts[0].replace("frame", "")
                else:
            ...
            if video_id not in self.video_groups:
                self.video_groups[video_id] = []
            self.video_groups[video_id].append(file)

        # Sort files within each group
        for video_id in self.video_groups:
            self.video_groups[video_id] = sorted(self.video_groups[video_id])

        # Create a list of video IDs
        self.video_ids = sorted(list(self.video_groups.keys()))

    def __len__(self):
        """Denotes the total number of samples"""
        return len(self.video_ids)

    def read_images(self, video_id):
        """Read images for a specific video"""
        X = []
        files = self.video_groups[video_id]

        # Select frames based on the frames parameter
        selected_files = []
        if len(files) > max(self.frames):
            for i in self.frames:
                if i < len(files):
                    selected_files.append(files[i])
        else:
            # If we don't have enough frames, use what we have
            selected_files = files[:min(len(files), len(self.frames))]

            # If we still need more frames, repeat the last frame
            while len(selected_files) < len(self.frames):
                selected_files.append(selected_files[-1] if selected_files else files[0])
        ...
        if X:
            ...
            # Create a dummy tensor if no frames were found
            X = torch.zeros((len(self.frames), 3, 224, 224))
    ...
    def __getitem__(self, index):
        ...
        # Load data
        X = self.read_images(video_id)  # (input) spatial images

        # For inference, we don't need labels, so return a dummy label
...
class Dataset_3DCNN(data.Dataset):
    ...
    def read_images(self, path, selected_folder, use_transform):
        X = []
        for i in self.frames:
            image = Image.open(os.path.join(path, selected_folder, 'frame_{:03d}.jpg'.format(i))).convert('L')
...
class Dataset_CRNN(data.Dataset):
    ...
    def read_images(self, path, selected_folder, use_transform):
        X = []
        for i in self.frames:
            image = Image.open(os.path.join(path, selected_folder, 'frame_{:03d}.jpg'.format(i)))
