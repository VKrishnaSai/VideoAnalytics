...
# ----------------------- Split Data -----------------------
# First split full dataset into train (70%) and test (30%)
...
def resume_training(model, train_dataloader, val_dataloader, criterion, optimizer, start_epoch, additional_epochs, device='cuda'):
...
def start_resumed_training():
    ...
    checkpoint_path = os.path.join(checkpoints_dir, "resume_checkpoint_epoch_20.pth")
    start_epoch = 0
    ...
    best_val_loss, metrics = resume_training(model, trainloader, valloader, criterion, optimizer, start_epoch, additional_epochs, device=device)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("fun_training_resume:app", host="0.0.0.0", port=8000, reload=True)