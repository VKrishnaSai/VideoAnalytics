...
# ----------------------- Logging Setup -----------------------
logging.basicConfig(
    filename='vivit_training_final.log',
    level=logging.INFO,
    format='%(asctime)s %(levelname)s: %(message)s'
)
logging.info("Final ViViT training started.")
...
data_root = "~/human_action_recognition/ucf_crime"
...
# ----------------------- Individual Plotting Functions (Option C) -----------------------
...
def start_final_training():
    ...
    # Plot individual training metrics (Option C)
...
async def on_startup():
    threading.Thread(target=start_final_training, daemon=True).start()
...
if __name__ == "__main__":
    ...
    uvicorn.run("fun_training_final:app", host="0.0.0.0", port=8000, reload=False)