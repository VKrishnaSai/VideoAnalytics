...
for testlist in testlist_files:
    with open(testlist, "r") as f:
        for line in f:
            ...
            if not line:
                continue
            # For testlist files, lines usually only contain the relative video path.
            # If labels are provided, adjust accordingly. Here, we assume labels are available.
            parts = line.split()
            if len(parts) == 2:
                video_rel_path, label_str = parts
                label = int(label_str) - 1  # Convert label to zero-indexed
            else:
                # If label is not present, infer from the directory name
                video_rel_path = parts[0]
                # Infer label from directory name using data_subset
