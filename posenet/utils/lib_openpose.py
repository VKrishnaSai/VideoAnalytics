...
def _set_config():
    ...
    # For tf 2.x, The following setting is needed
        ...
        try:
            for gpu in gpus:
                ...
                tf.config.experimental.set_virtual_device_configuration(
                    gpu,
                    [tf.config.experimental.VirtualDeviceConfiguration(memory_limit=MAX_FRACTION_OF_GPU_TO_USE * 1024)]
                )
...
def test_openpose_on_webcamera():
    ...
    webcam_reader = ReadFromWebcam(max_framerate=30)
    img_displayer = ImageDisplayer()
    
    # -- Initialize openpose detector    
    skeleton_detector = SkeletonDetector("mobilenet_thin", "656x368")
