...
class ReadFromVideo(object):
    def __init__(self, video_path, sample_interval=1):
        ...
        video_path = Path("./data_test/exercise.avi")  # Convert string to Path object
        self._video = cv2.VideoCapture(str(video_path))  # Use str() to convert Path object back to string for OpenCV
        print('weeee')
        print(str(video_path))  # Print the resolved absolute path
    ...
    def get_fps(self):
        ...
        if fps <= 0:
            warnings.warn("Invalid FPS detected. Setting default FPS to 30.")
            fps = 30  # Set a fallback FPS
        self._fps = fps
        return fps
