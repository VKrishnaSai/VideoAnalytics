...
# ----------------------- Logging Setup -----------------------
log_filename = f'vivit_training_25thApril.log'
logging.basicConfig(
...
NUM_WORKERS = 4  # Reduced for 16GB RAM - too many workers can cause memory issues
PIN_MEMORY = False  # Disabled to save RAM - with 4GB GPU, this can cause OOM
BENCHMARK = True
PREFETCH_FACTOR = 2
torch.backends.cudnn.benchmark = BENCHMARK
BATCH_SIZE = 8  # Reduced from 16 to fit in 4GB GPU memory
MIXED_PRECISION = True  # Keep this - saves GPU memory

# ----------------------- Custom Dataset with Caching -----------------------
class VideoDataset(Dataset):
    def __init__(self, video_paths, labels, num_frames=16, image_size=224, cache_size=50):  # Reduced cache for RAM constraints
        self.video_paths = video_paths
        self.labels = labels
        self.num_frames = num_frames
        self.image_size = image_size
        self.cache = {}
        self.cache_size = cache_size  # Reduced from 100 to 50 to save RAM
...
# ----------------------- Data Preparation -----------------------
def load_data(data_root="./UCF-101"):
