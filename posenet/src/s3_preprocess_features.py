...
def process_features(X0, Y0, video_indices, classes):
    ...
    print(f"Processing {len(X0)} skeleton frames into time-series features...")
    print(f"Window size: {WINDOW_SIZE} frames")
    start_time = time.time()
    
    ADD_NOISE = False
    if ADD_NOISE:
        print("Processing with noise augmentation...")
        X1, Y1 = extract_multi_frame_features(
            X0, Y0, video_indices, WINDOW_SIZE, 
            is_adding_noise=True, is_print=True)
        print("Processing without noise...")
        ...
        print(f"✅ Feature processing completed in {elapsed_time:.2f} seconds")
        print(f"   Generated {len(X)} feature vectors (with noise augmentation)")
        return X, Y
    else:
        ...
        elapsed_time = time.time() - start_time
        print(f"✅ Feature processing completed in {elapsed_time:.2f} seconds")
        print(f"   Generated {len(X)} feature vectors from {len(X0)} skeleton frames")
        return X, Y
