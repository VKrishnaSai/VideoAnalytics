...
def load_video(video_path, num_frames=16, image_size=224):
    ...
    # Generate safe sample indices
    sample_indices = np.linspace(0, frame_count - 1, num_frames, dtype=int)
    sample_indices = np.clip(sample_indices, 0, frame_count - 1)  # Ensures indices are valid

    for idx in sample_indices:
        cap.set(cv2.CAP_PROP_POS_FRAMES, idx)  # Jump to the frame index
        ret, frame = cap.read()
        if not ret:
