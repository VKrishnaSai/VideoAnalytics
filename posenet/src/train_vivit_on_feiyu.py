...
# ----------------------- Logging Setup -----------------------
log_filename = f'posenet_vivit_training_{datetime.now().strftime("%Y%m%d_%H%M")}.log'
logging.basicConfig(
...
logger.info("PoseNet ViViT Training started")
...
BATCH_SIZE = 8  # Reduced batch size for 10-second clips to avoid memory issues
...
# ----------------------- Data Preparation -----------------------
def load_data(data_root="../output/video_clips"):
    """
...
# ----------------------- Main Execution -----------------------
def main():
    parser = argparse.ArgumentParser(description="Train ViViT on PoseNet video clips")
    ...
    checkpoints_dir = f"posenet_checkpoints_{datetime.now().strftime('%Y%m%d_%H%M')}"
    ...
    train_videos, train_labels, val_videos, val_labels, test_videos, test_labels, action_classes = load_data("../output/video_clips")
    ...
    model = ViViT(image_size=224, patch_size=16, num_classes=len(action_classes), num_frames=16)
    ...
    # Updated checkpoint path for PoseNet training
    if args.checkpoint_dir:
        ...
        checkpoint_path = os.path.join("posenet_checkpoints_previous", "best_checkpoint.pth")
    ...
    logger.info("Starting training on video clips...")
    ...
    final_model_path = os.path.join(checkpoints_dir, "posenet_vivit_model_final.pth")
    if isinstance(model, nn.DataParallel):
    ...
    logger.info("PoseNet ViViT training complete!")
